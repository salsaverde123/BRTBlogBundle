...
class Util
{
    ...
    */
    public static function daysBetween(\DateTime $dateFrom, \DateTime $dateTo){
    ...
    */
    ...
    public static function getRangeDate($type, $string){
        ...
        switch ($type){
            case "from":
                return \DateTime::createFromFormat('Y-m-d H:i:s', $extract[0]);
                break;
            case "to":
                return \DateTime::createFromFormat('Y-m-d H:i:s', $extract[1]);
                break;
        }
    ...
    */
    ...
    public static function getDurationByFormat($string) {
        $testString = $string;
        $hours = 0;
        $minutes = 0;
        $seconds = 0;
        if($testString){
            $findHours = strpos($testString,"h");
            if($findHours!== false){
                if($findHours == 1){
                    $hours = (integer)substr($testString,0,1);
                } elseif($findHours == 2){
                    $hours = (integer)substr($testString,0,2);
                }
            }

            $findMinutes = strpos($testString,"min");
            if($findMinutes!== false){
                $secondMinute = substr($testString,$findMinutes-2,2);
                $integerSecondMinute = (integer)str_replace(" ","",$secondMinute);
                $minutes = $integerSecondMinute;
            }

            $findSeconds = strpos($testString,"sec");
            if($findSeconds!== false){
                $secondSeconds = substr($testString,$findSeconds-2,2);
                $integerSecondSecond = (integer)str_replace(" ","",$secondSeconds);
                $seconds = $integerSecondSecond;
            }
        }
    ...
    /**
     * Transforma un array de datos geolocalizados a un array con formato geojson
     *
     * @param array $points
     *
     * @return array
     */
    public static function transformToGeoJson(array $points){
        ...
        foreach ($points as $item) {
            ...
            $feature = array(
                ...
                'properties' => array(
                    'travel' => $item['travel_id'],
                    'speed' => $item['speed'],
                    'forceGx' => $item['forceGx'],
                    'forceGy' => $item['forceGy'],
                    'forceGz' => $item['forceGz']
                )
            );
