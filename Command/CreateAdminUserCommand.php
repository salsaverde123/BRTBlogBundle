...
class CreateAdminUserCommand extends ContainerAwareCommand {
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $output->writeln(["Usuario creado: admin", "", "Password: $passwordText"]);
        $em->persist($user);

        // Creamos el post inicial
        $postTitle = "Post de prueba";
        $post = new Post();
        $post->setPostTitle($postTitle);
        $post->setSlug(Util::slugify($postTitle));
        $post->setPostText("
        ...
        ");
        $post->setUser($user);
        $post->setPostSubtitle("Amazing fits...");
        $post->setCreated(new \DateTime('now'));
        $post->setUpdated(new \DateTime('now'));
        $em->persist($post);

        $output->writeln(["Nuevo post creado: ", $postTitle]);
